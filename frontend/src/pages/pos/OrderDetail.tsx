  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className={`px-2 py-0.5 rounded text-xs font-medium ${ORDER_STATUS_COLORS[order.status]}`}>{ORDER_STATUS_LABELS[order.status]}</span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-4">
        <div className="grid grid-cols-2 gap-px bg-gray-100">
          {[
            ...
            ['Table', order.table ? `T${order.table.tableNumber}` : '—'],
            ...
            <tbody className="divide-y divide-gray-100">
              {order.items.map(item => (
                <tr key={item.id}>
                  ...
                  <td className="px-4 py-2.5 text-right">₹{item.unitPrice}</td>
                  <td className="px-4 py-2.5 text-right font-medium">₹{item.lineTotal}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <ConfirmDeleteModal open={delOpen} onClose={() => setDelOpen(false)} onConfirm={() => remove.mutate()} loading={remove.isPending} message={`Delete order ${order.orderNumber}?`} />
    </div>
  );
};

...
export class CouponController {
  ...
  async createCoupon(req: Request, res: Response) {
    try {
      const { code, discount, active } = req.body as CreateCouponDto;
      if (!code || discount == null) {
        return res.status(400).json(errorResponse('Missing required fields', 'code and discount are required'));
      }
  ...
  async deleteCoupon(req: Request, res: Response) {
    try {
      ...
      if (coupon) {
        res.status(200).json(successResponse('Coupon deleted successfully', {}));
      } else {
        res.status(404).json(errorResponse('Coupon not found', 'Coupon not found'));
      }
    } catch (error: any) {

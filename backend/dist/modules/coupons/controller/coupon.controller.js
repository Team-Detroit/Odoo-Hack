...
class CouponController {
    ...
    async createCoupon(req, res) {
        try {
            const { code, discountType, discountValue, active, description, expiryDate, minPurchase, minOrders, monthsActive, newCustomerOnly } = req.body;
    ...
    async updateCoupon(req, res) {
        try {
            ...
            if (data.minPurchase != null)
                data.minPurchase = Number(data.minPurchase);
            if (data.minOrders != null)
                data.minOrders = Number(data.minOrders);
            if (data.monthsActive != null)
                data.monthsActive = Number(data.monthsActive);
    ...
    async validateCoupon(req, res) {
        try {
            ...
            if (!couponCode || !customerEmail) {
                return res.status(400).json((0, response_util_1.errorResponse)('Missing required fields', 'coupon and customerEmail are required'));
            }
            // 1. Coupon exists
            const coupon = await prisma_1.default.coupon.findUnique({ where: { code: couponCode.toUpperCase() } });
            if (!coupon) {
                return res.status(404).json((0, response_util_1.errorResponse)('Coupon code does not exist', 'Coupon code does not exist'));
            }
            // 2. Coupon active
            if (!coupon.active) {
                return res.status(400).json((0, response_util_1.errorResponse)('Coupon is not active', 'Coupon is not active'));
            }
            // Check usage limits
            if (coupon.usageCount >= coupon.maxUsageCount) {
                return res.status(400).json((0, response_util_1.errorResponse)('Coupon limit reached', `This coupon has reached its maximum usage limit of ${coupon.maxUsageCount}.`));
            }
            // 3. Not expired
            if (coupon.expiryDate && new Date(coupon.expiryDate) < new Date()) {
                return res.status(400).json((0, response_util_1.errorResponse)('Coupon has expired', 'Coupon has expired'));
            }
            // Find all customer records sharing this email address
            const customersWithEmail = await prisma_1.default.customer.findMany({
            ...
            if (coupon.minPurchase != null && totalPurchase < coupon.minPurchase) {
                return res.status(400).json((0, response_util_1.errorResponse)('Customer not eligible', `Customer total purchase (₹${totalPurchase.toFixed(2)}) is less than required minimum (₹${coupon.minPurchase.toFixed(2)})`));
            }
            if (coupon.minOrders != null && orderCount < coupon.minOrders) {
                return res.status(400).json((0, response_util_1.errorResponse)('Customer not eligible', `Customer order count (${orderCount}) is less than required minimum (${coupon.minOrders})`));
            }
            if (coupon.monthsActive != null) {
                if (orderCount === 0) {
                    return res.status(400).json((0, response_util_1.errorResponse)('Customer not eligible', `Customer has no previous orders to satisfy active customer rule`));
                }
                ...
                if (lastOrder < thresholdDate) {
                    return res.status(400).json((0, response_util_1.errorResponse)('Customer not eligible', `Customer last order was not within the required ${coupon.monthsActive} months`));
                }
            }
            if (coupon.newCustomerOnly && orderCount > 0) {
                return res.status(400).json((0, response_util_1.errorResponse)('Customer not eligible', `Coupon is only valid for new customers (0 orders)`));
            }
            ...
            if (customersWithEmail.length > 0) {
                ...
                const redemption = await prisma_1.default.couponRedemption.findFirst({
                    where: {
                        couponId: coupon.id,
                ...
                if (redemption) {
                    return res.status(400).json((0, response_util_1.errorResponse)('Coupon already redeemed', 'Coupon has already been redeemed by this customer'));
                }
            }
            // Calculate discount
            ...
            res.status(500).json((0, response_util_1.errorResponse)('Failed to validate coupon', error.message));
        }
    }
    async runEmailCampaignForCoupon(coupon) {
        try {

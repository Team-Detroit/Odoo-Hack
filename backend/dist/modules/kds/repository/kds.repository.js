...
class KdsRepository {
    async getAllKitchenTickets() {
        return prisma_1.default.kitchenTicket.findMany({ include: { order: { include: { items: true } } } });
    }
    async getKitchenTicketById(id) {
        return prisma_1.default.kitchenTicket.findUnique({ where: { id }, include: { order: { include: { items: true } } } });
    }

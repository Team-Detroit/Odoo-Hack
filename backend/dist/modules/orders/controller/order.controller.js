...
class OrderController {
    ...
    async createOrder(req, res) {
        try {
            const { sessionId, tableId, customerId, subtotal, discount, tax, total } = req.body;
            ...
            const order = await this.orderService.createOrder({ sessionId, tableId, customerId, subtotal, discount, tax, total });
            if (socket_1.io) {
    ...
    async deleteOrder(req, res) {
        try {
            ...
            if (order) {
                res.status(200).json((0, response_util_1.successResponse)('Order deleted successfully', {}));
            }
            else {
                res.status(404).json((0, response_util_1.errorResponse)('Order not found', 'Order not found'));
            }
